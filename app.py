...
def scrape_all_products(store_url):
    ...
    while True:
        ...
        new_links = ["https://gumroad.com" + a["href"] if not a["href"].startswith("http") else a["href"] for a in product_elements if "gumroad.com/l/" in a["href"]]
        
        if not new_links:
            break  # Stop when no more new products are loaded
        
        product_links.update(new_links)
        page += 1
        time.sleep(1)  # Avoid overloading Gumroad
